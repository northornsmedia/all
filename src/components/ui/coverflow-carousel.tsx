    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-12 outline-none ring-ring focus-visible:ring-2 active:cursor-grabbing"
          style={{
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                onClick={() => goTo(index)}
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-muted shadow-2xl transition-shadow duration-300 will-change-transform cursor-pointer border border-white/10 hover:border-white/20",
                  cardClassName,
                )}
                style={{ width: "var(--cf-card)" }}
              >
                {/* Visual gloss overlay for vinyl / physical cover feel */}
                <div className="absolute inset-0 bg-gradient-to-tr from-black/40 via-transparent to-white/15 pointer-events-none z-10" />
                <img
                  src={slide.src}
                  alt={slide.alt}
                  draggable={false}
                  ...
                  className="h-full w-full select-none object-cover object-top transition-transform duration-500 hover:scale-105"
                />
                {/* Bottom title & website overlay */}
                {slide.title && (
                  <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/95 via-black/60 to-transparent z-20 flex flex-col items-start pointer-events-none">
      ...
      {showCaption && active?.title && (
        ...
        >
          <div className="inline-flex items-center gap-2 mb-1.5 px-3 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider bg-primary/15 text-primary border border-primary/20">
            {active.subtitle || "Album Showcase"}
          </div>
          ...
          {active.meta && active.meta.length > 0 && (
            <div className="mt-6 w-full max-w-sm rounded-xl p-4 glass-panel border border-border shadow-xl">
              <dl className="grid grid-cols-3 gap-2 text-center text-xs">
                {active.meta.map((row) => (
                  <div key={row.label} className="flex flex-col gap-1 p-2 rounded-lg bg-background/40">
                    <dt className="text-muted-foreground uppercase text-[10px] tracking-wider font-medium">{row.label}</dt>
                    <dd className="font-semibold text-foreground text-xs sm:text-sm truncate">{row.value}</dd>
                  </div>
                ))}
              </dl>
            </div>
